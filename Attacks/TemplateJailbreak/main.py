...
config = get_config()
# MAX_ALLOWED_ITERATION_PER_QUESTION = config.MAX_ALLOWED_ITERATION_PER_QUESTION    # Not used here because we have 78 templates that we loop over
...
def run(model_name):
    ...
    df1 = pd.read_excel(jailbreak_path)     # Read the 78 templates
    prompts_templates = df1['text']
    train_data = pd.read_csv(file_path)     # Read the 60 questions
    ...
    for question in questions:
        ...
        for template in prompts_templates:
            # Insert the question into the template and repeat it
